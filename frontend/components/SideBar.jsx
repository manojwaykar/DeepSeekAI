    ...
    z-50 max-md:absolute max-md:h-screen ${expand ? 'p-4 w-64' : 'md:w-20 w-0 max-md:overflow-hidden'}`}>
        <div>
            <div className={`flex ${expand ? "flex-row gap-10" : "flex-col items-center gap-8"}`}>
                <img className={expand ? "w-36" : "w-10"} src={expand ? assets.logo_text : assets.logo_icon} alt=''/>
                ...
                h-9 w-9 aspect-square rounded-lg cursor-pointer'>
                    <img src={assets.menu_icon} className='md:hidden' alt=''/>
                    <img src={expand ? assets.sidebar_close_icon : assets.sidebar_icon} className='hidden md:block w-7' alt=''/>
                    <div className={`absolute w-max ${expand ? 'left-1/2 -translate-x-1/2 top-12' : "-top-12 left-0"} opacity-0 
                    group-hover:opacity-100 transition bg-black
            ...
            <button onClick={createNewChat}
             className={`mt-8 flex items-center justify-center cursor-pointer 
                ${expand ?  "bg-indigo-800 hover:opacity-90 rounded-2xl gap-2 p-2.5 w-max" : "group relative w-9 h-9 mx-auto hover:bg-gray-500/30 rounded-lg"}`}>
                <img className={expand ? "w-6" : "w-7"} src={expand? assets.chat_icon : assets.chat_icon_dull} alt=''/>
                <div className='absolute w-max -top-12 -right-12 opacity-0 group-hover:opacity-100 
                transition bg-black text-white text-sm px-3 py-3 rounded-lg shadow-lg pointer-events-none'>
                    New chat
            ...
            <div className={`mt-8 text-white/25 text-s ${expand? "block" : "hidden"}`}>
                <p className='my-1'>Recents</p>
                { chats.map((chat)=> <ChatLabel name={chat.name} id={chat._id} openMenu={openMenu} key={chat._id} setOpenMenu={setOpenMenu}/>)}
            </div>
        </div>
        <div>
            <div className={`flex items-center cursor-pointer group relative ${expand? "gap-1 text-white/80 text-sm p-2.5 border border-primary rounded-lg hover:bg-white/10 cursor-pointer" : "h-10 w-10 mx-auto hover:bg-gray-500/30 rounded-lg"}`}>
                <img className={expand? "w-5" : "w-6 mx-auto"} src={expand? assets.phone_icon : assets.phone_icon_dull} alt=''/>
                <div className={`absolute -top-60 pb-8 ${!expand && "-right-40"} opacity-0 group-hover:opacity-100 hidden group-hover:block transition`}>
                    <div className='relative w-max bg-black text-white text-sm p-3 rounded-lg shadow-lg'>
                        <img src={assets.qrcode} alt='' className='w-44'/>
            ...
            gap-3 text-white/60 text-sm p-2 mt-2 cursor-pointer`}>
                {user? <UserButton/> : <img src={assets.profile_icon} alt='' className='w-7'/>}
